...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase().groupBy(x => x).mapValues(s => s.length()).toList.sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if(occurrences.isEmpty) List(List())
      ...
      val currentFreq = occurrences.head._2
      val listOfListOfNewComb = for(x <- 1 to currentFreq) yield tailCombinations.map(oldC => (currentChar, x) :: oldC)
      listOfListOfNewComb.flatten.toList ++ tailCombinations
    }
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagramsStep(o: Occurrences): List[Sentence] =
      if (o.isEmpty) List(List())
      else (for {
        comb <- combinations(o)
        word <- (dictionaryByOccurrences withDefaultValue List())(comb)
        transformedRemainder <- sentenceAnagramsStep(subtract(o, comb))
      } yield word :: transformedRemainder).toList

    sentenceAnagramsStep(sentenceOccurrences(sentence))
  }
}
